<template>
  <q-page padding>
    <div v-if="!objectId" class="text-grey text-center q-mt-xl">
      在地图上选择一个对象或玩家以查看/编辑属性
    </div>
    <div v-else-if="!object" class="text-negative text-center q-mt-xl">目标已不存在</div>
    <template v-else>
      <div class="text-h6 q-mb-md">{{ object.name || object.objectId }}</div>
      <property-field
...
const blocks = computed(
  () => parseComponentData(object.value?.componentData ?? '') ?? [],
);
